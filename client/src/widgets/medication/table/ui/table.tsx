...
const Table: FC<Props> = ( { medications, ...props } ) => {
    ...
    <table className="w-full min-w-200 text-left table-fixed" {...props}>
      <thead>
        <tr>
          ...
          <th className="p-3">
            <button onClick={() => setParams( { sort: 'unit' } )} className="inline-flex items-center gap-1 cursor-pointer">
              <span>Unit</span>
              <ChevronsUpDown className="w-3 h-3" />
            </button>
          </th>
      ...
      <tbody>
        {medications.map( medication => <Row key={medication.id} medication={medication} /> )}
      </tbody>
    </table>
  );
};
