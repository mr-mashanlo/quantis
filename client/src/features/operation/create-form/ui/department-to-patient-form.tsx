...
import { Cat, ChevronDown, Hospital, OctagonAlert } from 'lucide-react';
...
export const DepartmentToPatientForm: FC<Props> = ( { className, description = 'Lorem ipsum dolor sit amet', ...props } ) => {
  const form = useCreateOperationForm();
  const { departments } = useDepartments( { limit: '10000' } );
  const { patients } = usePatients( { limit: '10000' } );
  ...
  const filteredPatients = patientQuery === '' ? patients.data?.data : patients.data?.data.filter( patient => patient.name?.toLowerCase().includes( patientQuery.toLowerCase() ) );
  ...
  return (
    <form onSubmit={handleFormSubmit} className={twMerge( 'w-full sm:w-165 relative', className )} {...props}>
      ...
      <div className="w-full sm:w-90 mx-auto grid gap-4 sm:gap-5">
        ...
        <form.Field name="toPatient" validators={{ onChange: CreateOperationFormSchema.shape.toPatient }}>
          {field =>
            <Field className="block relative">
              <Combobox value={field.state.value ?? null} onChange={e => e ? field.handleChange( e ) : null} onClose={() => setPatientQuery( '' )}>
                <ComboboxInput displayValue={( user: { name: string } ) => user?.name} onChange={e => setPatientQuery( e.target.value )} placeholder="To" className="peer w-full p-4 pl-11 rounded-xl bg-zinc-100 dark:bg-zinc-800 placeholder:text-zinc-300 dark:placeholder:text-zinc-600 focus:bg-transparent data-[error=true]:outline-rose-500" />
                <ComboboxOptions anchor="bottom" className="w-(--input-width) p-1 z-10 bg-white dark:bg-zinc-800 rounded-md border border-zinc-200 dark:border-zinc-800/50 transition duration-300 ease-out [--anchor-gap:--spacing(1)] focus:outline-none data-closed:scale-95 data-closed:opacity-0">
                  {filteredPatients?.map( patient => <ComboboxOption key={patient.id} value={patient} className="w-full px-3 py-2 flex items-center gap-2 rounded-md cursor-pointer hover:bg-zinc-100 dark:hover:bg-zinc-950">{patient.name}</ComboboxOption> )}
                </ComboboxOptions>
              </Combobox>
              {!field.state.meta.isValid ? <OctagonAlert className="w-4 h-4 stroke-red-500 absolute top-1/2 left-4 -translate-y-1/2" aria-hidden="true" /> : <Cat className="w-4 h-4 stroke-zinc-300 dark:stroke-zinc-600 peer-focus:stroke-black dark:peer-focus:stroke-zinc-400 absolute top-1/2 left-4 -translate-y-1/2" aria-hidden="true" />}
              <ChevronDown className="w-4 h-4 stroke-zinc-300 absolute top-1/2 right-4 -translate-y-1/2" />
            </Field>
          }
        </form.Field>
...
export default DepartmentToPatientForm;
