import { Button, Field, Listbox, ListboxButton, ListboxOption, ListboxOptions } from '@headlessui/react';
import { useSelector } from '@tanstack/react-form';
import { ChevronDown, OctagonAlert, User } from 'lucide-react';
import { type DetailedHTMLProps, type FC, type FormHTMLAttributes, type SubmitEvent } from 'react';
import { twMerge } from 'tailwind-merge';

import { type Operation, UpdateOperationSchema } from '@/entities/operation';

import { useUpdateOperationForm } from '../model/hook';

interface Props extends DetailedHTMLProps<FormHTMLAttributes<HTMLFormElement>, HTMLFormElement> {
  operation: Operation
};

export const UpdateOperationForm: FC<Props> = ( { operation, className, ...props } ) => {
  const form = useUpdateOperationForm( operation );
  ...
  return (
    <form onSubmit={handleFormSubmit} className={twMerge( 'w-full sm:w-165 relative', className )} {...props}>
      <h3 className="mb-5 text-xl text-center font-bold">Update operation</h3>
      <div className="w-full sm:w-90 mx-auto grid gap-4 sm:gap-5">
        <form.Field name="status" validators={{ onChange: UpdateOperationSchema.shape.status }}>
          {field =>
            <Field className="block relative">
              <Listbox value={field.state.value} onChange={e => field.handleChange( e )}>
                ...
                <ListboxOptions anchor="bottom" className="w-(--button-width) p-1 z-10 bg-white dark:bg-zinc-800 rounded-md border border-zinc-200 dark:border-zinc-800/50 transition duration-300 ease-out [--anchor-gap:--spacing(1)] focus:outline-none data-closed:scale-95 data-closed:opacity-0">
                  <ListboxOption value="DRAFT" className="w-full px-3 py-2 flex items-center gap-2 rounded-md cursor-pointer hover:bg-zinc-100 dark:hover:bg-zinc-950">DRAFT</ListboxOption>
                  <ListboxOption value="COMPLETED" className="w-full px-3 py-2 flex items-center gap-2 rounded-md cursor-pointer hover:bg-zinc-100 dark:hover:bg-zinc-950">COMPLETED</ListboxOption>
                  <ListboxOption value="CANCELLED" className="w-full px-3 py-2 flex items-center gap-2 rounded-md cursor-pointer hover:bg-zinc-100 dark:hover:bg-zinc-950">CANCELLED</ListboxOption>
                </ListboxOptions>
              </Listbox>
